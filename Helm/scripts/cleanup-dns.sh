...
# is a shared personal domain also used by other, unrelated projects, so
# ExternalDNS is deliberately never allowed to auto-delete anything there).
# That means ExternalDNS itself will NEVER clean these records up on its own
# after the Ingress is deleted, no matter how long you wait - this script does
# that cleanup directly via the Route53 API instead.
#
...
# cluster_name written to SSM by terraform/ssm-outputs.tf), so it never
# touches another project's records sharing the same hosted zone.
#
